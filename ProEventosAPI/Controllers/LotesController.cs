    ...
    public class LotesController : ControllerBase
    {
        ...
        public async Task<IActionResult> Delete(int eventoId, int loteId)
        {
            try
            {
                ...
                return await _loteService.DeleteLote(lote.EventoId, loteId) ? Ok(new { message = "Lote Deletado" }) : throw new Exception("Ocorreu um erro não expecificadp ao tentar deletar lote");
            }
            catch (Exception ex)
            {
                return this.StatusCode(StatusCodes.Status500InternalServerError,
